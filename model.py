...
def build_stoi(vocab):
    ...
    to_index = {v:i for i,v in enumerate(vocab)}
    return to_index
...
def decode_ids(ids, itos):
    ...
    return "".join(str(itos[i]) for i in ids)
...
def softmax_overflow_demo(large_value):
    ...
    overflowed = True if exp == np.inf else False
    return {'naive_exp':exp, 'overflowed': overflowed}
...
def stable_softmax_2d_rowwise(logits):
    ...
    summ = np.sum(exp,axis=1,keepdims=True)
    return exp/summ
...
def encode_corpus_to_int_array(text, stoi):
    ...
    return np.array([encode_string(t,stoi) for t in text]).astype(int).squeeze(-1)
...
def sample_random_batch_offsets(data_len, block_size, batch_size, rng):
    ...
    valid = data_len - block_size
    return rng.integers(0,valid,batch_size)
...
def stack_x_batch(data, offsets, block_size):
    ...
    array = [slice_x_at_offset(data, offsets[i], block_size) for i,o in enumerate(offsets)]
...
def stack_y_batch(data, offsets, block_size):
    ...
    return np.stack([slice_y_at_offset(data, offsets[i],block_size) for i,o in enumerate(offsets)])

# Step 44 - get_batch
def get_batch(data, block_size, batch_size, rng):
    # TODO: package one training batch (X, Y) of shape (batch_size, block_size) from data using rng.
    indexes = sample_random_batch_offsets(len(data), block_size,batch_size,rng)
    x = stack_x_batch(data, indexes, block_size)
    y = stack_y_batch(data, indexes, block_size)
    return x,y
...
def sample_next_token(p_matrix, current_id, rng):
    ...
    return rng.choice(int(p_matrix.shape[1]),p=p_matrix[current_id])
...
def generate_sequence(p_matrix, start_id, length, rng):
    ...
    array = np.array([start_id])
    for i in range(length-1):
        array = np.append(array, sample_next_token(p_matrix,array[-1],rng))
    return array
...
def sum_negative_log_probs(p_matrix, data):
    ...
    return -sum(log_prob_of_pair(p_matrix,data[d],data[d+1]) for d in range(len(data)-1))
...
def one_hot_encode_batch(ids, vocab_size):
    ...
    matrix = np.zeros((len(ids),vocab_size))
...
def cross_entropy_loss(probs, targets):
    ...
    return -np.average(array_log(gather_correct_token_probs(probs,targets)))
...
def compute_dlogits(probs, targets):
    ...
    matrix = probs.copy()
    matrix[np.arange(len(targets)),targets] -=1
    return matrix / len(targets)
...
def train_neural_bigram_loop(w, data, block_size, batch_size, learning_rate, num_steps, log_every):
    ...
    for i in range(num_steps):
        ...
        batch_loss = 0
        for x,y in zip(X,Y):
            calc = run_one_training_step(w,x,y,learning_rate)
            w=calc['w']
            batch_loss+=calc['loss']
        batch_loss /= len(X)

        if count % log_every == 0:
            loss.append(batch_loss)
        count +=1
    return {'w':w, 'loss_history':loss}
...
def sample_from_neural_bigram(w, start_id, num_tokens, itos):
    ...
    for i in range(num_tokens):
        logits = forward_logits_lookup(w,string[-1])[None,:]
        probs = logits_to_probs_rowwise(logits)
        next_id = rng.choice(len(probs[0]),p=probs[0,:])
        #next_id = max(probs[0,:])
        string.append(next_id)
    
    return decode_ids(string, itos)
...
def layernorm_forward_variance(x, mean):
    ...
    mid = x-mean
    #return np.sum(mid*mid,axis=-1,keepdims=True)/len(x[-1])
